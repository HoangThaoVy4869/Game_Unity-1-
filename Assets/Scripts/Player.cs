...
public class Player : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (m_isShooted)
		{
			...
			GameGunManager.Ins.UpdateFireRate(m_curFireRate / fireRate);
		}
	...
	void Shot(Vector3 mousePos)
	{
		...
		if(hits != null && hits.Length > 0)
		{
			for (int i = 0; i < hits.Length; i++)
			{
				...
				if(hit.collider && (Vector3.Distance((Vector2)hit.collider.transform.position, (Vector2)mousePos) <= 0.4)){
					Birds bird = hit.collider.GetComponent<Birds>();
		...
		AudioCotroller.Ins.PlaySound(AudioCotroller.Ins.shooting);
	}
}
